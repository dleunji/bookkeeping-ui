...
import { changeInput, changeSecond, changeStatusCode } from '../../modules/ordinary';
...
const OrdinaryContainer = () => {
  const { totalAmount, step1, currentStep, registered, step2, statusCode, second } = useSelector(
    ({ pgCard, ordinary }) => ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      setTimeout(() => {
        ...
        parentWindow.postMessage(statusCode, 'http://localhost:3000/pg-card');
        window.close();
      }, 5000);
    }
  }, [statusCode]);
